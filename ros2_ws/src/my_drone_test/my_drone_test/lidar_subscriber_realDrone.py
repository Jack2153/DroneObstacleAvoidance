...
class FloodFillNode(Node):
    ...
    def initialize(self):
        ...
        # Create the Sensors and specify the GPIO PIN of the Raspberry Pi to which the sensor is connected
        sensor_front = GroveUltrasonicRanger(6)
        sensor_left = GroveUltrasonicRanger(25)
        sensor_back = GroveUltrasonicRanger(5)
        sensor_right = GroveUltrasonicRanger(26)
    ...
    def add_obstacle(self):
        ...
        # Define the distances of the different sensors. get_distance() is in centimeters.
        distance_front = int(round(sensor_front.get_distance()))/100 # Rounded distance in Meters
        distance_left = int(round(sensor_left.get_distance()))/100 # Rounded distance in Meters
        distance_back = int(round(sensor_back.get_distance()))/100 # Rounded distance in Meters
        distance_right = int(round(sensor_right.get_distance()))/100 # Rounded distance in Meters
        
        # Call the functions to add the obstacles with the right distance in the right direction
        add_obstacle_front(distance_front)
        add_obstacle_left(distance_left)
        add_obstacle_back(distance_back)
        add_obstacle_right(distance_right)
            
    # Add an obstacle in fornt of the drone
    def add_obstacle_front(distance):
    ...
    # Add an obstacle to the left of the drone
    def add_obstacle_left(distance)
    ...
    # Add an obstacle in the back of the drone
    def add_obstacle_back(distance)
        obstacle_x = self.robot_x - distance
        obstacle_y = self.robot_y
    ...
    # Add an obstacle to the right of the drone
    def add_obstacle_right(distance)
        obstacle_x = self.robot_x
        obstacle_y = self.robot_y - distance
